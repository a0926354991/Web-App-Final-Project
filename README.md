...
| **課程比較** | 勾選 2-3 門課 → 全螢幕並排比較適合度 / PTT 樣本 / 評量方式 / 課程要求；每欄可單獨 ✕ 移除 |
| **使用者資訊** | 科系 + 5 軸能力 + 甜度偏好 + loading 偏好 + 興趣 tag |
| **我的科系** | 下拉選系所（沿用 `/departments` 清單）；推薦時本系課（含合開）加權，並標「⭐ 你本系開的課」 |
...
| Method | Path | 用途 | 需 auth |
|---|---|---|---|
| GET | `/me/profile` | 取得偏好（含科系，不存在則回預設）| ✓ |
| PUT | `/me/profile` | 寫入偏好（upsert，含科系）| ✓ |
| GET | `/me/recommendations?limit=N` | Top N 推薦（排除已修；有填科系則本系課優先 + 保底）| ✓ |
...
- **興趣分加成**：使用者系名出現在課程開課系所字串中（含「A系 / B學程 / C所」這種多系所合開課）→ 視為本系課，興趣成份加分；推薦理由標「⭐ 你本系開的課」。
- **候選池納入本系課**：`/me/recommendations` 原本只從「有 PTT 評價」的課挑，本系課多半無評價會整批漏掉；現在把本系課也納入候選，缺評價者用冷啟動代理 stats（同課號前綴 / 同系所均值）評分並標 `is_estimated`。
- **本系保底名額**：Top N 約 80% 名額保留給本系課（依分數取最高的），其餘留給全校最高適合度課 ——「本系優先 + 全校探索」。未填科系則 quota=0，完全是原本的全校排序。

### 設計取捨

- 排序候選以「有 PTT 結構化評價」的課號為主；其他課缺資料無法直接評估甜度 / loading / 推薦（本系課例外，會以冷啟動代理值納入）。
